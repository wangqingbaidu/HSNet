...
void update_network_use_flag(network net, int* flag)
{
    int i;
    int update_batch = net.batch*net.subdivisions;
    float rate = get_current_rate(net);
...
float train_network_datum(network net, float *x, float *y)
{
    ...
    network_state state;
    *net.seen += net.batch;
    //flag acts as whether to use [ith] layer of not
    state.index = 0;
    state.net = net;
    state.input = x;
    state.delta = 0;
...
float train_network_datum_use_flag(network net, float *x, float *y)
{
#ifdef GPU
    if(gpu_index >= 0) return train_network_datum_gpu(net, x, y);
#endif
    ...
    *net.seen += net.batch;
    ...
    state.truth = y;
    state.train = 1;
    forward_network_use_flag(net, state, flag, 1);
    backward_network_use_flag(net, state, flag);
    float error = get_network_cost(net);
    if(((*net.seen)/net.batch)%net.subdivisions == 0) update_network_use_flag(net, flag);
    free(flag);
    return error;
}
...
//        float err = train_network_datum(net, X, y);
        //use new method to train network
        float err = train_network_datum_use_flag(net, X, y);
        sum += err;
    }
    free(X);
    free(y);
...
{
    int size = get_network_output_size(net); //Get output number of last none cost layer.
    float *out = get_network_output(net);
