...
network parse_network_cfg(char *filename)
{
    ...
    while(n){
        ...
        if(n){
        	layer* prev_l = &l;
			if (prev_l->type == COST)
				prev_l = find_prev_in_shortcut(&net, count - 1);
			if (!prev_l)
			{
				error("There must be a convolutional layer before shortcut");
			}
			params.h = prev_l->out_h;
			params.w = prev_l->out_w;
			params.c = prev_l->out_c;
			params.inputs = prev_l->outputs;
        }
    }   
    free_list(sections);
...
network* parse_network_cfg2pointer(char *filename)
{
    list *sections = read_cfg(filename);
    node *n = sections->front;
    if(!n) error("Config file has no sections");
    network* net = make_network2pointer(sections->size - 1);
    net->gpu_index = gpu_index;
    size_params params;
